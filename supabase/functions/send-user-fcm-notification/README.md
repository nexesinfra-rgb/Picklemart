# Send User FCM Notification Edge Function

This Edge Function sends Firebase Cloud Messaging (FCM) push notifications to regular users using the FCM HTTP v1 API.
...
curl -X POST 'https://bgqcuykvsiejgqeiefpi.supabase.co/functions/v1/send-user-fcm-notification' \
  ...
  -d '{
    ...
    "title": "Order Updated",
    "message": "Your order #12345 status changed to Processing",
    "order_id": "uuid-here",
    "order_number": "12345",
...
```typescript
{
  ...
  user_id?: string          // Optional: to send to specific user (if omitted, sends to all users)
}
```
...
- ✅ Sends notifications to all active user FCM tokens (or specific user if `user_id` provided)
...
- The function sends notifications to all active user FCM tokens stored in the `user_fcm_tokens` table (or a specific user if `user_id` is provided)
- Failed notifications are logged but don't fail the entire batch
- Each notification is sent individually to each user device
