...
SQL_FILE = r"c:\Users\Venky\Downloads\Pickle mart(25-02-2026)\sm\DISABLE_CONFLICTS.sql"
...
def apply_fix():
    ...
    try:
        sftp = ssh.open_sftp()
        sftp.put(SQL_FILE, "/tmp/disable_conflicts.sql")
        sftp.close()
    except Exception as e:
    ...
    # 3. Exec SQL
    print("Applying fix (Removing old conflicts)...")
    ssh.exec_command(f"docker cp /tmp/disable_conflicts.sql {container_id}:/tmp/disable_conflicts.sql")
    cmd_exec = f"docker exec -i {container_id} psql -U postgres -d postgres -f /tmp/disable_conflicts.sql"
