...
SQL_FILE = r"c:\Users\Venky\Downloads\Pickle mart(25-02-2026)\sm\FIX_TRIGGER_AMBIGUITY.sql"
...
def apply_fix():
    ...
    try:
        sftp = ssh.open_sftp()
        sftp.put(SQL_FILE, "/tmp/fix_trigger.sql")
        sftp.close()
    except Exception as e:
    ...
    # 3. Exec SQL
    print("Applying fix...")
    ssh.exec_command(f"docker cp /tmp/fix_trigger.sql {container_id}:/tmp/fix_trigger.sql")
    cmd_exec = f"docker exec -i {container_id} psql -U postgres -d postgres -f /tmp/fix_trigger.sql"
