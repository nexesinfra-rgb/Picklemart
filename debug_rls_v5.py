...
BEGIN
    ...
    VALUES ('{victim}', 'confirmed', 10.00, 10.00, 0, 0, '{{}}', 'TEST-' || floor(random()*100000)::text);
...
"""
            ...
            sftp = ssh.open_sftp()
            with sftp.file("/root/test_rls_v5.sql", "w") as f:
                f.write(sql_sim)
            
            cmd_exec = f"cat /root/test_rls_v5.sql | docker exec -i {db_container} psql -U postgres -d postgres"
            stdin, stdout, stderr = ssh.exec_command(cmd_exec)
            print(stdout.read().decode().strip())
            print(stderr.read().decode().strip()) 
            
            ssh.exec_command("rm /root/test_rls_v5.sql")
        else:
